...
impl GraphExecutorImpl {
    ...
    pub fn execute(&self, graph: &Graph) -> Result<GraphResult, String> {
        ...
        if !validation.valid {
            let error_msg = validation.errors.iter()
                ...
                .collect::<Vec<_>>()
                .join("; ");
            return Ok(GraphResult::error(format!("Validation failed: {}", error_msg)));
        }
        ...
        for node_instance in sorted_nodes {
            ...
            let node = match self.registry.create_node(&node_instance.node_type, node_instance.id.clone()) {
                Ok(n) => n,
            ...
            let params = match serde_wasm_bindgen::to_value(&node_instance.parameters) {
                Ok(v) => v,
                Err(e) => return Ok(GraphResult::error(format!("Failed to serialize parameters: {:?}", e))),
            };
            ...
            let outputs = match node.execute(inputs, params) {
                ...
                Err(e) => {
                    ...
                    return Ok(GraphResult::error(format!("Node '{}': {}", node_instance.id, error_msg)));
                }
            };
    ...
    fn get_node_inputs(
        ...
        let def = self.registry.get_definition(&node.node_type)
        ...
        for edge in &graph.edges {
            if edge.to_node == node.id {
                ...
                let data = context.get_output(&edge.from_node, edge.from_port)
                    .ok_or_else(|| format!(
        ...
        for (idx, input) in inputs.into_iter().enumerate() {
            result.push(input.ok_or_else(|| format!(
                "Missing input at port {} for node '{}'",
                idx, node.id
            ))?);
        }
    ...
    fn topological_sort(&self, graph: &Graph) -> Result<Vec<NodeInstance>, String> {
        ...
        // Build graph
        for edge in &graph.edges {
            adj_list.entry(edge.from_node.clone())
                .or_default()
...
mod tests {
    ...
    fn test_topological_sort_simple_chain() {
        ...
        graph.add_node(NodeInstance::new("n1", "passthrough", Position::new(0.0, 0.0)));
        graph.add_node(NodeInstance::new("n2", "passthrough", Position::new(100.0, 0.0)));
    ...
    fn test_topological_sort_multiple_paths() {
        ...
        graph.add_node(NodeInstance::new("n1", "passthrough", Position::new(0.0, 0.0)));
        graph.add_node(NodeInstance::new("n2", "passthrough", Position::new(100.0, 0.0)));
        graph.add_node(NodeInstance::new("n3", "passthrough", Position::new(100.0, 100.0)));
        graph.add_node(NodeInstance::new("n4", "passthrough", Position::new(200.0, 0.0)));
